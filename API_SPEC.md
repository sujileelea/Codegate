...
> v2.1 추가: **음성 응원 편지 신설**(§3 — 우체통·편지함, 24시간 휘발/프리미엄 영구 보관), **똑똑이 음성 일기 신설**(§4 — 기기 내 STT, 음성 원본 미수집, 별도 동의), **가족 선물하기**(§6 — 상점 구매에 수령인 지정, 가족 커머스·B2B 접점), **프리미엄 구독**(§11 — 3티어 entitlement + 서버 게이트 402), **케어 푸시 신설**(§8 무활동 룰 확정 — 3일 무활동 alert + §10 디바이스 등록·APNs 발송, 티어 무관), 상점 카테고리·제휴 확장(§6), 교육 콘텐츠 `premium` 플래그(§5), 가족 지갑 충전 내역·충전자 표시(§7), 형제자매(guardian) 초대 명시(§3), 동의 코드 `third_party_research` 예약(§1.7).
...
| subscription `status` (§11) | `active`, `expired`, `demo` |
...
- **자금 흐름 남용 방어**: 앱은 부모-자식 관계를 공식 증명하지 않으므로 자금 이동 채널(세탁·보이스피싱·탈세)로 악용될 수 있다 — 실결제 전환 시 ⓐ 출금은 시니어 본인 명의 계좌만 ⓑ 충전·출금·현금성 선물 한도 ⓒ 신규 연결 냉각 기간 ⓓ 이상 거래 탐지(FDS) ⓔ 환불은 원결제수단만을 적용한다. 통제 정본은 보안 문서.md §4.6(M-1~M-8), 수치는 부록 A-7 ⑩.
...
응답 200: `user_id, role, status, real_name, display_name, email, phone, birth_date, gender, timezone, families[], consents[], entitlements`
...
요청(모두 선택): `real_name`, `display_name`, `email`, `phone`, `birth_date`, `gender`, `timezone`, `marketing_consent`, `voice_diary_consent`, `voice_diary_share_with_family`, `third_party_research_consent` → 응답은 GET `/me`와 동일.
...
|---|---|
| `senior_id` | 수신 시니어 |
| `audio` | 음성 파일 — **AAC(m4a), 최대 60초·5MB** (형식·길이·크기 서버 검증, 초과 400) |
| `transcript` | 자녀 기기 on-device STT 전사, ≤ 1,000자 (편지지 텍스트) |
...
```json
{"letters": [
  {"cheer_id": "…", "from_display_name": "김민준", "visibility": "named",
...
```json
{
  ...
  "streak_days": 2,
  "mailbox": {"unread_count": 1, "latest_from_display_name": "김민준", "latest_sent_at": "…"}
}
```
...
```
- `transcript` ≤ 4,000자. **DEK 암호화 저장**, 보존 기간 만료 시 원문 파기·파생 지표만 보존(기간은 부록 A-7 ⑦).
...
{"contents": [
  {"content_id": "…", "emoji": "🩺", "title": "혈압, 이렇게 관리해요", "format": "article",
   "estimated_seconds": 120, "reward_points": 15, "completed": false, "premium": false}
]}
```
...
{"content_id": "…", "title": "…", "format": "article",
 "body_md": "## 혈압 관리\n\n…", "video_url": null, "reward_points": 15, "completed": false, "premium": false}
```
...
- **실출금 전환의 전제(확정 원칙 — 보안 문서.md §4.6)**: ⓐ 출금 계좌는 **시니어 본인 명의만**(1원 인증/계좌실명조회로 가입 실명 대조 — 검증 없이 실출금 출시 금지) ⓑ 출금 일/월 한도 적용 ⓒ 신규 가족 연결 후 냉각 기간·계좌 변경 후 48시간 보류 ⓓ 선불전자지급수단 해당 여부·전금법 등록·AML 검토 선행. 한도 수치는 부록 A-7 ⑩.
...
{"purchase_id": "…", "item_id": "…", "price_points": 5000, "balance": 29500,
 "recipient": {"user_id": "…", "display_name": "김민준"},
...
- **현금성 선물 유속 제한(확정 원칙 — 보안 문서.md §4.6 M-3)**: 실발급 전환 시 `gifticon`·`health`·`partner` 카테고리의 선물 구매에 **월 한도(소액)**를 적용한다 — 충전→선물→현금화("깡")가 주간 보상 상한을 우회하는 유일한 대량 경로이기 때문. 비현금성(`deco`·`theme`·`widget`)은 제외. 신규 가족 연결 후 냉각 기간에는 현금성 선물 불가. 수치는 부록 A-7 ⑩.
...
{"gifts": [
  {"gift_id": "…", "item": {"name": "카페 기프티콘", "emoji": "☕", "category": "gifticon"},
   "from_display_name": "김순자", "to_display_name": "김민준",
...
- **실결제 전환 시 충전 월 한도**(가족당·충전자당)를 적용한다(보안 문서.md §4.6 M-3 — 수치는 부록 A-7 ⑩). 미사용 충전 포인트의 환불은 **원결제수단으로만**(제3자 계좌 환불 경로 없음, M-2).
...
```json
{
  "entries": [
    {"charge_id": "…", "amount_krw": 100000, "status": "completed",
     "charged_by": {"user_id": "…", "display_name": "김민준", "relation_label": "큰아들"},
     "created_at": "…"}
  ],
  "next_cursor": null
}
...
- 발송 채널: APNs 직접 발송 또는 FCM 경유 중 택1(운영 준비 시 확정 — APNs 인증 키(.p8)·Team ID·번들 ID 설정 필요, A-1 Firebase 프로젝트 공유와 함께 합의).
...
24. **④ 구독 상품 구성·가격**: `sub_premium_monthly` 가격, 연간 상품 여부, 무료 체험 기간, `premium_ai` 티어의 차별 기능(추천 고도화·심화 리포트)과 출시 시점 — 게이트 경계(§11 표)는 확정, 상품 정책만 대기.
...
29. **⑨ 음성 응원 편지 스토리지**: 오브젝트 스토리지 선정(배포 플랫폼과 함께 — 암호화 at rest·24h 수명주기 규칙·서명 URL), 오디오 형식·상한 최종 확정(현 계약: AAC 60초·5MB·서명 URL 15분), MVP 데모 범위(스토리지 확정 전 오디오 없이 전사만 운영할지) 결정.
30. **⑩ 자금 흐름 남용 방어 수치·절차**(실결제 전환 ②③의 전제 — 원칙은 확정, 보안 문서.md §4.6): 충전 월 한도(가족당·충전자당), 출금 일/월 한도, 현금성 선물 월 한도, 신규 가족 연결 냉각 기간(후보 72시간~7일), FDS 룰 임계값, 본인 명의 계좌 검증 방식(1원 인증 vs 계좌실명조회 — 오픈뱅킹/펌뱅킹 사업자 선정), 선불전자지급수단 해당 여부·전금법 등록·AML 법률 검토 결과, 선택적 관계 인증(고한도 티어) 도입 여부.
