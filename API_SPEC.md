...
> **확정 정책**: 1P = 1원 고정 · 일일 미션 5개(홈 진행률 분모 5) · 초대 코드 4자리 · 로그인은 Apple + 카카오(휴대폰 OTP 미제공) · 활동 달력 제공(§4, 시니어·자녀 공용) · 게임 시드 = 기획서 최종 6종(§1.8) · **걸음 기능 제거** · 반응시간·힌트 계측 필수(§4).
...
| ledger `reason_type` | `mission_complete`, `checkin`, `edu_complete`, `goal_bonus`, `streak_bonus`, `redeem`, `withdraw`, `adjust` |
...
| code | 이름 | type | domain | 보상 | 예상 시간 | 기획서 순위 | iOS 상태 |
|---|---|---|---|---|---|---|---|
| `digit_symbol` | 숫자·기호 짝맞추기 | game | attention | 30P | 180s | 1위 | ❌ 신규 구현 |
| `spot_difference` | 다른 그림 찾기 | game | visuospatial | 35P | 240s | 2위 | ❌ 신규 구현 |
| `sequence_recall` | 순서 기억하기 | game | memory | 40P | 180s | 3위 | ✅ 구현 완료 |
| `signal_tap` | 신호에 맞춰 누르기 | game | attention | 30P | 120s | 4위 | ❌ 신규 구현 |
| `word_sort` | 단어·그림 분류하기 | game | language | 35P | 240s | 5위 | ⚠️ 목업(완성 필요) |
| `daily_order` | 일상 순서 맞추기 | game | executive | 40P | 240s | 6위 | ⚠️ 목업(완성 필요) |
...
- 신규 게임의 보상 포인트·난이도 기준은 기획서 §10 논의 항목 — 위 표 값은 잠정치.
...
```json
{
  ...
  "display_name": "김순자",
  "featured_mission": {
    "assignment_id": "…", "status": "assigned", "source": "ai", "rank": 1,
    "mission": {"mission_id": "…", "code": "sequence_recall", "mission_type": "game",
                "title": "순서 기억하기", "emoji": "🧠", "difficulty": 2,
                "cognitive_domain": "memory", "reward_points": 40, "estimated_seconds": 180}
  },
  "missions_total": 5,
...
```
- 각 항목에 `emoji`, `estimated_seconds`, `reward_points`, assignment `status` 포함 — 카드의 "완료" 체크 표시는 `status == "completed"`.
...
```json
{
  "mission": {"mission_id": "…", "code": "sequence_recall", "title": "순서 기억하기"},
...
```json
{
  ...
  "recommendations": [
    {"mission_id": "…", "code": "word_sort", "title": "단어 분류하기", "emoji": "🧩",
     "mission_type": "game", "cognitive_domain": "language", "reward_points": 35,
     "score": 0.87, "reason": "언어 인지에 도움이 되는 게임을 추천해요"}
  ]
}
```
- 하루 1회 배치(Qwen 3 32B@Groq)가 사전 계산한 결과 조회(항상 빠름). 홈 `featured_mission`과 리포트 `recommendations`의 원천.
